...
SELECT DAYNAME(event_date) AS day_of_week, COUNT(*) AS num_sales, AVG(eth_price) FROM pricedata
GROUP BY day_of_week
ORDER BY num_sales;

/* 6) Construct a column that describes each sale and is called summary. The sentence should include 
who sold the NFT name, who bought the NFT, who sold the NFT, the date, and what price it was sold 
for in USD rounded to the nearest thousandth.
 ...
 SELECT CONCAT(name, " was sold for $", ROUND(usd_price,2), " to ", seller_address, " from ",buyer_address, 
" on ", event_date, ".") AS Summary FROM pricedata;
...
SELECT ROUND(eth_price,-2) AS bucket, COUNT(*) AS count,
RPAD('', COUNT(*), '*') AS Bar
FROM pricedata
GROUP BY bucket
ORDER BY bucket;

/* 9) Return a unioned query that contains the highest price each NFT was bought for and a 
new column called status saying “highest” with a query that has the lowest price each NFT was 
bought for and the status column saying “lowest”. The table should have a name column, 
...
/* 10) What NFT sold the most each month / year combination? Also, what was the name and the price in USD? 
Order in chronological format.     */
SELECT YEAR(event_date), MONTH(event_date), name, COUNT(*) AS total_num_sales, MAX(usd_price) FROM pricedata
GROUP BY YEAR(event_date), MONTH(event_date), name
ORDER BY YEAR(event_date), MONTH(event_date) ASC;

/* 11) Return the total volume (sum of all sales), round to the nearest hundred on a monthly basis (month/year).  */
SELECT YEAR(event_date) AS year, MONTHNAME(event_date) AS month, 
ROUND(SUM(usd_price),-2) AS total_sales_volume_usd FROM pricedata
GROUP BY year, month
ORDER BY year, month ASC;

/* 12) Count how many transactions the wallet "0x1919db36ca2fa2e15f9000fd9cdc2edcf863e685"had over this time period. */
SELECT COUNT(*) AS num_transactions FROM pricedata 
...
/* 13) Create an “estimated average value calculator” that has a representative price of the collection every day 
based off of these criteria:
 - Exclude all daily outlier sales where the purchase price is below 10% of the daily average price
 - Take the daily average of remaining transactions
 a) First create a query that will be used as a subquery. Select the event date, the USD price, and the 
 average USD price for each day using a window function. Save it as a temporary table.
 b) Use the table you created in Part A to filter out rows where the USD prices is below 
 10% of the daily average and return a new estimated value which is just the daily average of the filtered data.  */

CREATE TEMPORARY TABLE temp_daily_avg_price
SELECT event_date, usd_price, AVG(usd_price) OVER(PARTITION BY event_date) AS daily_avg_usd_price FROM pricedata;

SELECT event_date, AVG(daily_avg_usd_price) AS estimated_average_value FROM temp_daily_avg_price
